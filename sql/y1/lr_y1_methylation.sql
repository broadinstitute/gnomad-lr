-- Direct-canonical raw per-sample Total/combined rows. The table remains inactive until
-- a separate serving decision; no ancillary pointer is written by finalization.
...
ORDER BY (ancillary_run_id, task_id, attempt_id, lease_id, chrom, position, sample_id);
