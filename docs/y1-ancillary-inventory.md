...
No entry is currently authorized for Y1 serving.
...
| Recombination rate | **Shared reference candidate**, cohort-independent if GRCh38 map/version is pinned. | Current `fetchRecombinationRate` static source identity is not recorded here. | No Rust loader or ClickHouse table. | `Query.recombination_rate`, haplotype views. | Unscoped by reference-map version and not cached with ancillary identity. Blocked from a provenance-complete Y1 bundle until pinned. |
